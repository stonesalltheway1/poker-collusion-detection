"""exp_cb_pairmodel: CatBoost GPU pair risk model on engine pair features.
Mirrors src/exp005_pairmodel.py with CatBoost backend (GPU oblivious trees).
Usage: python src/exp_cb_pairmodel.py [exp_id] [W_U] [rounds] [train_window]
"""
import json
import os
import sys
...
EXP = sys.argv[1] if len(sys.argv) > 1 else "exp045_cb"
...
def main():
    t0 = time.time()
    print(f"[{EXP}] Starting CatBoost pair model training (ROUNDS={ROUNDS}, W_U={W_U}, TRAIN={TRAIN})...", flush=True)
    ...
    y = train["y"].to_numpy().astype(np.int32)
    ...
    for k in range(5):
        ...
        m = cb.CatBoostClassifier(iterations=ROUNDS, learning_rate=0.06, depth=6, l2_leaf_reg=3.0,
                                  task_type="GPU", verbose=0, random_seed=seed + k)
        m.fit(X[tr], y[tr], sample_weight=w[tr])

        oof_full[idx_f] = m.predict_proba(Xf[idx_f])[:, 1]
        oof_w2a[idx_w] = m.predict_proba(Xw[idx_w])[:, 1]
    ...
    for k in range(5):
        ...
        mb = lgb.train(bparams, lgb.Dataset(Xp[tr], yp[tr], weight=wt, feature_name=feats), 300)
        post_pos[pfold == k] = mb.predict(Xp[pfold == k])
        post_C[wfold == k] = mb.predict(Xw[wfold == k])
    ...
    for s_idx in range(3):
        t_fit = time.time()
        m_all = cb.CatBoostClassifier(iterations=ROUNDS, learning_rate=0.06, depth=6, l2_leaf_reg=3.0,
                                      task_type="GPU", verbose=0, random_seed=seed + s_idx * 100)
        m_all.fit(X, y, sample_weight=w)
        risk += m_all.predict_proba(Xe)[:, 1] / 3.0
        print(f"[{EXP}] Eval seed {s_idx} trained in {time.time() - t_fit:.1f}s", flush=True)

    cwall = np.bincount(yp, minlength=3)
    mb_final = lgb.train(bparams, lgb.Dataset(Xp, yp, weight=(len(yp) / (3 * cwall))[yp], feature_name=feats), 300)
    post_e = mb_final.predict(Xe)
